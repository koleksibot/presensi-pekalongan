...
class pegawai extends system\Controller {
    ...
    protected function index() {
        $data['title'] = 'PEGAWAI';
        $data['subtitle'] = 'Halaman Master - Pegawai';
        $data['table_title'] = 'Tabel Master Pegawai';
        $data['breadcrumb'] = '<nav class="custom">
          <div class="nav-wrapper">
            <div class="col s12">
              <a href="'.$this->link().'" class="breadcrumb">Beranda</a>
              <a href="#!" class="breadcrumb">Master Pegawai</a>
            </div>
          </div>
        </nav>';
    ...
    public function form() {
        $input = $this->post(true);
        if ($input) {
            $data['form_title'] = (!(empty($input['id_pegawai']))) ? 'Ubah Data Pegawai' : 'Tambah Data Pegawai';
