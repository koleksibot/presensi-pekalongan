...
class laporan extends system\Controller {
    ...
    protected function tabelindex() {
        ...
        if ($input) {
            ...
            $data['verifikator'] = $this->login['kdlokasi'];
            $data['code'] = 'superior_agency';
            $data['lokasi'] = $this->laporan_service->getChildLokasi($data);
            $data['laporan'] = $this->laporan_service->getAllLaporan($data)['value'];
            $data['namabulan'] = comp\FUNC::$namabulan1[$data['bulan']];
    ...
    protected function verified() {
        ...
        $data['code'] = 'superior_agency';
        $data['verifikator'] = $this->login['kdlokasi'];
        $data['pil_satker'] = $this->laporan_service->getChildLokasi($data);
        $data['breadcrumb'] = '<a href="' . $this->link() . '" class="breadcrumb white-text" style="font-size: 13px;">'
                . 'Index</a><a class="breadcrumb white-text" style="font-size: 13px;">'
    ...
    public function getPilLaporan() {
        $input = $this->post(true);
        if ($input) {
            $input['code'] = 'superior_agency';
            $input['verifikator'] = $this->login['kdlokasi'];
            $lokasi = $this->laporan_service->getChildLokasi($input);
...
//            comp\FUNC::showPre($data);exit;
    ...
    protected function tabelbatal() {
        ...
        if ($input) {
            ...
            // if($this->login['username'] == 'acil@adminkota') {
            $data['verifikator'] = $this->login['kdlokasi'];
            $data['code'] = 'superior_agency';
            $data['satker'] = $this->laporan_service->getChildLokasi($data);
            $data['dataTabel'] = $this->laporan_service->getVerLaporan2($data);
