...
class laporan_service extends system\Model {
    ...
    public function getChildLokasi($input = array()) {
        parent::setConnection('db_presensi');
        $idKey = array($input['code'], $input['verifikator']);
        $getChild = $this->getdata('SELECT child FROM tb_parent WHERE 1 AND `code` = ? AND parent = ?', $idKey)['value'];
        $child = implode('\', \'', array_column($getChild, 'child'));
        
        parent::setConnection('db_pegawai');
        $data = $this->getData('SELECT kdlokasi, singkatan_lokasi FROM tref_lokasi_kerja WHERE kdlokasi IN (\'' . $child . '\') ORDER BY singkatan_lokasi', []);
        if ($data['count'] > 0) {
            foreach ($data['value'] as $val) {
                $result[$val['kdlokasi']] = $val['singkatan_lokasi'];
            }
...
//        $result['query'] = '';
    ...
    public function getArraymodAll($input, $laporan = []) {
        ...
        foreach ($data['value'] as $mod) {
            ...
            if ($input['tingkat'] == 4 && $mod['flag_operator_kota'] != 2)
                $verified = null;
    ...
    public function getKepala($kdlokasi) {
        //checkparent
        parent::setConnection('db_pegawai');
        ...
        if ($get['count'] > 0 && $get['value'][0]['kdlokasi_parent']) {
            $kdlokasi = $get['value'][0]['kdlokasi_parent'];
        }

        parent::setConnection('db_presensi');
        $get = $this->getData('SELECT * FROM tb_pengguna WHERE kdlokasi = ? AND grup_pengguna_kd = ?', [$kdlokasi, 'KDGRUP02']);
...
//        parent::setConnection('db_presensi');
//        $get = $this->getData("SELECT * FROM tb_laporan WHERE pin_absen IS NULL AND bulan = ? AND tahun = ?", [$data['bulan'], $data['tahun']]);
//        return $get;
//    }
    
    public function getAllLaporan($data) { // Modif zaenal
        parent::setConnection('db_presensi');
        $idKey = array($data['bulan'], $data['tahun'], $data['verifikator'],'superior_agency');
        $get = $this->getData("SELECT a.* FROM tb_laporan a "
                . "JOIN tb_parent b ON a.`kdlokasi` = b.`child` "
                . "WHERE a.`pin_absen` IS NULL AND a.`bulan` = ? AND a.`tahun` = ? AND b.`parent` = ? AND b.`code` = ?", $idKey);
        return $get;
    }

    public function getVerLaporan($data) {
        parent::setConnection('db_presensi');
        $params = [$data['bulan'], $data['tahun']];

        $get = $this->getData("SELECT * FROM tb_laporan WHERE bulan = ? AND tahun = ? ORDER BY dt_sah_final IS NULL, dt_sah_final DESC, dt_ver_admin_kota IS NULL, dt_ver_admin_kota DESC, dt_sah_kepala_opd IS NULL, dt_sah_kepala_opd DESC", $params);
        return $get['value'];
    }

    public function getVerLaporan2($data) {
        parent::setConnection('db_presensi');
        $whereIn = 'AND kdlokasi IN ("' . implode('","', array_keys($data['satker'])) . '")';
        $params = [$data['bulan'], $data['tahun']];

        $get = $this->getData("SELECT * FROM tb_laporan WHERE bulan = ? AND tahun = ? " . $whereIn . " ORDER BY dt_sah_final IS NULL, dt_sah_final DESC, dt_ver_admin_kota IS NULL, dt_ver_admin_kota DESC, dt_sah_kepala_opd IS NULL, dt_sah_kepala_opd DESC", $params);
        return $get['value'];
    }
