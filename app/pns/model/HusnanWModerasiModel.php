...
//FUNC::husnanWVarDump($sql);
    ...
    public function getInfoModerasi($pinAbsen, $mid, $isFinal = false) {
        return $this->getDaftarMod($pinAbsen, $mid, $isFinal)[0];
    }

    public function updateModerasi($data, $kodeLokasi) {
    ...
    public function getDaftarMod($pinAbsen, $mid = null, $isFinal = false, $dateAwal = null, $dateAkhir = null) {
        ...
        if ($mid === null) {
            $where .= " AND tanggal_awal BETWEEN '" . $dateAwal . "' AND '" . $dateAkhir . "' AND tanggal_akhir BETWEEN '" . $dateAwal . "' AND '" . $dateAkhir . "'";

            $sql = "SELECT tmod.*, tjm.nama_jenis, tkp.ket_kode_presensi, tkp.pot_kode_presensi FROM tb_moderasi tmod INNER JOIN tb_kode_presensi tkp ON tmod.kode_presensi = tkp.kode_presensi INNER JOIN tb_jenis_moderasi tjm ON tmod.kd_jenis = tjm.kd_jenis WHERE tmod.pin_absen = ? " . $where . " ORDER BY tmod.id DESC";
            $params = [$pinAbsen];
        } else {
            $sql = "SELECT tmod.*, tjm.nama_jenis, tkp.ket_kode_presensi, tkp.pot_kode_presensi, tkp.kode_presensi, tgrup.nama_grup_pengguna AS grup_pemohon FROM tb_moderasi tmod INNER JOIN tb_kode_presensi tkp ON tmod.kode_presensi = tkp.kode_presensi INNER JOIN tb_jenis_moderasi tjm ON tmod.kd_jenis = tjm.kd_jenis INNER JOIN tb_grup_pengguna tgrup ON tmod.usergroup = tgrup.kd_grup_pengguna WHERE tmod.pin_absen = ? AND tmod.id = ? " . $where;
            $params = [$pinAbsen, $mid];
        }
        ...
        parent::setConnection('db_pegawai');
        $sql = "SELECT tpeg.nipbaru, CONCAT(gelar_depan, ' ', namapeg, ' ', gelar_blkg) AS nama_lengkap, pin_absen, singkatan_lokasi FROM texisting_personal tperson INNER JOIN texisting_kepegawaian tpeg ON tperson.nipbaru = tpeg.nipbaru INNER JOIN tref_lokasi_kerja tlokasi ON tpeg.kdlokasi = tlokasi.kdlokasi WHERE pin_absen = ?";
        $daftarPegawai = $this->getData($sql, [$pinAbsen])["value"];
        $daftarModPegawai = [];
    ...
    public function getCurrentPns($nip) {
        ...
        $result = $this->getData($sql, [$nip]);
        return ($result['count'] > 0) ? $result["value"][0] : [];
    }
