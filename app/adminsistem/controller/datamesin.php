...
class datamesin extends system\Controller {
    ...
    protected function index() {
        $data['title'] = 'Data Kehadiran Pada Mesin Fingerprint';
    ...
    public function getPilLokasiFromKelLokasi() {
        $input = $this->post(true);
        if ($input) {
            $data = $this->pegawai_service->getData('SELECT kdlokasi, singkatan_lokasi FROM tref_lokasi_kerja WHERE (kd_kelompok_lokasi_kerja = ?) AND (status_lokasi_kerja = ?)', array($input['id_kelompok'], '1'));
            if ($data['count'] > 0) {
