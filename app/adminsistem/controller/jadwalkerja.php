...
class jadwalkerja extends system\Controller {
    ...
    protected function tabel() {
        ...
        if ($input) {
            ...
            // comp\FUNC::showPre($data['dataPersonil']);
...
//            comp\FUNC::showPre($data);
    ...
    protected function simpan() {
        ...
        if ($input) {
            ...
            } else if (empty($input['sdate']) || empty($input['edate'])) {
				$error_msg = array('title' => 'Gagal', 'message' => 'Tanggal awal atau akhir harus diisi.', 'status' => 'error');
            } else {
...
//            comp\FUNC::showPre($tabelShiftDetail);
    ...
    public function getPilLokasiFromKelLokasi() {
        $input = $this->post(true);
        if ($input) {
            $data = $this->pegawai_service->getData('SELECT kdlokasi, singkatan_lokasi FROM tref_lokasi_kerja WHERE (kd_kelompok_lokasi_kerja = ?) AND (status_lokasi_kerja = 1) ORDER BY singkatan_lokasi', array($input['id_kelompok']));
            if ($data['count'] > 0) {
