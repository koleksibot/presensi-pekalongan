...
//        $result['query'] = $data['query'];
    ...
    public function getLastUpdate($input) {
        $result = array();
        $data = $this->getData('SELECT id_mesin, MAX(tanggal_log_presensi) AS last_update FROM tb_log_presensi WHERE id_mesin IN ('. $input .') '
                . 'GROUP BY id_mesin');
        if ($data['count'] > 0) {
            foreach ($data['value'] as $val) {
                $result[$val['id_mesin']] = $val['last_update'];
            }
        }
        return $result;
    }
    ...
    public function getPilShift($cari = array()) {
        ...
        $query = 'SELECT * FROM tb_shift WHERE (kdlokasi = ? OR kdlokasi = "system")';
    ...
    public function getTabelMesinX($data) {
        $idKey = array();
        $page = $data['page'];
...
//        $result['query'] = '';
    ...
    public function getTabelMesin($data) {
        ...
        $posisi = ($page - 1) * $batas;
        $jmlData = $this->getData($j_mesin, $idKey);
        $dataArr = $this->getData($q_mesin . ' LIMIT ' . $posisi . ', ' . $batas, $idKey);
