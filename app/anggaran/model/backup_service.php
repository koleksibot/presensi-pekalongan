...
//        $result['query'] = '';
    ...
    private function save_laporan($input, $tbinduk) {
     ...
     public function getDataPersonilTpp($data) {
        ...
        $query = 'SELECT pin_absen, pp.*, npwp, gol_jbtn, nominal_tp, tunjangan_jabatan, s.urutan_sotk, v.kd_jabatan_khusus, v.kd_tp, v.kd_ruang_jab,
            ...
            JOIN view_pegawai v ON v.nipbaru = pp.nipbaru ' . $q_cari;
    ...
    private function save_personil($input, $tbinduk, $rekap, $w_presensi) {
        ...
        foreach ($pegawai as $peg) {
            ...
            if ($tbpersonil['error'] && $w_presensi) {
                ...
                $tbpresensi = $this->save_presensi($rekap, $peg, $tbpersonil['inserted_id']);
                if (!$tbpresensi['error'])
                    return $tbpresensi;
            }
        }
    ...
    public function save_presensi($rekap, $peg, $personil_id) {
        parent::setConnection('db_backup');
