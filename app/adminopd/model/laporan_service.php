...
//        $unverified = $this->getData($j_query)['count'];
    ...
    public function getLaporan($data) {
        ...
        if ($data['tingkat'] == 1 && $data['bulan'] == 2 && $data['tahun'] == 2018) {
            ...
            $get = $this->getData("SELECT * FROM tb_pengguna WHERE kdlokasi = ?
                AND (grup_pengguna_kd = 'KDGRUP01' || grup_pengguna_kd = 'KDGRUP02')
            ", [$data['kdlokasi']]);
            ...
            foreach ($get['value'] as $i) {
                if ($i['grup_pengguna_kd'] == 'KDGRUP01')
                    $laporan['ver_admin_opd'] = $i['nipbaru'];
                if ($i['grup_pengguna_kd'] == 'KDGRUP02')
                    $laporan['sah_kepala_opd'] = $i['nipbaru'];
            }
            $get['count'] = 1;
            $get['value'][0] = $laporan;
        }
    ...
    public function getDataVarLaporan($kode) {
        ...
        if ($data['count'] > 0) {
            $keys = array_column($data['value'], 'kode_variabel');
    ...
    public function getAdminopd($kdlokasi, $nipbaru) {
        //checkparent
        parent::setConnection('db_pegawai');
        $get = $this->getData('SELECT * FROM tref_lokasi_kerja WHERE kdlokasi = ?', [$kdlokasi]);

        parent::setConnection('db_presensi');
        $get = $this->getData('SELECT * FROM tb_pengguna WHERE kdlokasi = ? AND grup_pengguna_kd = ? AND nipbaru = ?', [$kdlokasi, 'KDGRUP01', $nipbaru]); //ADMINOPD
