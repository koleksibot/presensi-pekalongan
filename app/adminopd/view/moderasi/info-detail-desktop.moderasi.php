    ...
    <div id="detail-info" class="col s12">
        <table class="highlight bordered stripped" width="100%">
            ...
            <tr>
                ...
                <td>
                    <ul class="no-s">
                        <?php foreach ($lampiran as $key => $val): ?>
                            <li class="listLampiran<?= $key ?>">
                                <a href="<?= $this->link('upload/moderasi/dokumen/' . $val) ?>" target="blank"><?= $val ?></a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </td>
...
<script>
    //$(".detailAction").hide();
<?php
if (is_null($flag_kepala_opd) && $lockReport == false) {
    ?>
