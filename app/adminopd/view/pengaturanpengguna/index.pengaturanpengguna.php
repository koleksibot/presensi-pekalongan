    ...
    <script>
        (function ($) {
            ...
            $(document).on("submit", "#frmInput", function () {
                ($("#status_pengguna").is(":checked")) ? $("#status_pengguna").val("enable") : $("#status_pengguna").val("disable");
                app.simpan(this);
            });

            $(document).on("click", ".btnHapus", function () {
                $("#myConfirmModalLabel").html("Konfirmasi Hapus");
                $("#data-confirm").html("Apakah anda ingin menghapus data <span class='blue-text' for=''><b>"+$(this).attr("id")+"</b></span> ?");
