...
class shift extends system\Controller {
    ...
    protected function jamkerja() {
        ...
        if ($input) {
            ...
            $data['dataShiftDetail'] = $this->presensi_service->getTabelKrit('tb_shift_detail', array('id_shift' => $input['id_shift'], 'batas'=> '100'), 'startdays');
    ...
    protected function formInputShift() {
        $input = $this->post(true);
        if ($input) {
            $data['id_satker'] = $this->login['kdlokasi'];
            $data['pil_unitShift'] = $this->presensi_service->getPilUnitShift();
            $data['dataTabel'] = $this->presensi_service->getDataKrit('tb_shift', array('id_shift' => $input['id_shift']));
...
//            comp\FUNC::showPre($data);
    ...
    protected function formInputJamKerja() {
        ...
        if ($input) {
            ...
            $data['pil_jamKerja'] = $this->presensi_service->getPilKrit('tb_jam_kerja', array('order' => 'jam_masuk'), $dt['opsi']);
