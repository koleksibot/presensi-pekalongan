...
class HusnanWModerasiModel extends system\Model {
    ...
    {
        $infoMod = $this->getDaftarVerMod($mid, $isFinal)["daftarModPegawai"];
    ...
    {        
        ...
        $params = [
            "flag_kepala_kota" => $data["flag"],
            "dt_flag_kepala_kota" => date("Y-m-d H:i:s"),
            "catatan_kepala_kota" => $catatan,
            "dt_last_modified" => date("Y-m-d H:i:s")
        ];

        if (empty($catatan) || $catatan === "") {
            $sql = "SELECT id, flag_kepala_opd FROM tb_moderasi WHERE id IN(".implode(",", $mids).")";

            //edited by daniek
            $sql = "SELECT id, flag_kepala_opd FROM tb_moderasi WHERE id IN(".implode(",", $mids).")  AND kdlokasi = '".$data['kdlokasi']."'";
            $mods = $this->getData($sql, [])["value"];

            //FUNC::husnanWVarDump($mods);

            foreach ($mods as $key => $mod) {
                if ($mod["flag_kepala_opd"] === "0") {
                    $mods[$key]["flag_kepala_opd"] = "3";
                } elseif ($mod["flag_kepala_opd"] === "1") {
                    $mods[$key]["flag_kepala_opd"] = "2";
                }
            }

            foreach ($mods as $mod) {
                //added by daniek
                if ($mod["flag_kepala_opd"] == NULL)
                    continue;

                $this->husnanWUpdate("tb_moderasi", [
                    "flag_kepala_opd" => $mod["flag_kepala_opd"],
                    "status_final" => $mod["flag_kepala_opd"] === "2" ? "DISAHKAN" : "DIBATALKAN"
                ], "id = ".$mod["id"]);
            }
        }
    ...
    public function getDaftarVerModOri($mid = null, $isFinal = false, $page = 1)
    {
        ...
        if ($mid === null || !is_numeric($mid)) {
            ...
            FROM tb_moderasi tmod INNER JOIN tb_kode_presensi tkp ON tmod.kode_presensi = tkp.kode_presensi INNER JOIN tb_jenis_moderasi tjm ON tmod.kd_jenis = tjm.kd_jenis WHERE flag_operator_opd IS NOT NULL AND flag_kepala_opd IS NOT NULL AND flag_operator_kota IS NOT NULL ".$where;
            ...
            FROM tb_moderasi tmod INNER JOIN tb_kode_presensi tkp ON tmod.kode_presensi = tkp.kode_presensi INNER JOIN tb_jenis_moderasi tjm ON tmod.kd_jenis = tjm.kd_jenis WHERE flag_operator_opd IS NOT NULL AND flag_kepala_opd IS NOT NULL AND flag_operator_kota IS NOT NULL ".$where." ORDER BY jml DESC, pin_absen, flag_kepala_opd, tanggal_awal LIMIT ".$posisi.", ".$batas;
        } else {
            $sql = "SELECT tmod.*, tjm.nama_jenis, tkp.ket_kode_presensi, tkp.pot_kode_presensi, tgrup.nama_grup_pengguna AS grup_pemohon FROM tb_moderasi tmod INNER JOIN tb_kode_presensi tkp ON tmod.kode_presensi = tkp.kode_presensi INNER JOIN tb_jenis_moderasi tjm ON tmod.kd_jenis = tjm.kd_jenis INNER JOIN tb_grup_pengguna tgrup ON tmod.usergroup = tgrup.kd_grup_pengguna WHERE tmod.id = ? AND flag_operator_opd IS NOT NULL AND flag_kepala_opd IS NOT NULL AND flag_operator_kota IS NOT NULL ".$where;
            $params = [$mid];
        }
    ...
    public function getDaftarVerMod($input)
    {
        ...
        if (isset($input['kdlokasi']) && ($input['kdlokasi'] === null || !is_numeric($input['kdlokasi']))) {
            ...
            if (isset($input['status'])) {
                switch ($input['status']) {
                    case 'semua':
                        ...
                        break;
                    case 'sah':
                        $c_status = 'AND flag_kepala_kota = "2"';
                        break;
                    case 'null':
                        $c_status = 'AND flag_kepala_kota IS NULL';
                        break;
                }
                $where .= $c_status . ' ';
            }
            ...
            $sql = "SELECT tmod.*, tjm.nama_jenis, tkp.ket_kode_presensi, tkp.pot_kode_presensi, (SELECT COUNT(*) FROM tb_moderasi tm WHERE tm.pin_absen = tmod.pin_absen) AS jml
                FROM tb_moderasi tmod INNER JOIN tb_kode_presensi tkp ON tmod.kode_presensi = tkp.kode_presensi INNER JOIN tb_jenis_moderasi tjm ON tmod.kd_jenis = tjm.kd_jenis WHERE flag_operator_opd IS NOT NULL AND flag_kepala_opd IS NOT NULL AND flag_operator_kota IS NOT NULL ".$where." ORDER BY jml DESC, pin_absen, flag_kepala_opd, tanggal_awal";
        }
    ...
    {
        parent::setConnection('db_presensi');
        $sql = "SELECT COUNT(id) AS total FROM tb_moderasi WHERE flag_operator_opd IS NOT NULL AND flag_kepala_opd IS NOT NULL AND flag_operator_kota IS NOT NULL AND flag_kepala_kota IS NULL AND kdlokasi = ?";
...
//FUNC::husnanWVarDump($total);
    ...
    {
        parent::setConnection('db_presensi');
        $sql = "SELECT COUNT(id) AS total FROM tb_moderasi WHERE flag_operator_opd IS NOT NULL AND flag_kepala_opd IS NOT NULL AND flag_operator_kota IS NOT NULL AND flag_kepala_kota IS NULL";
    ...
    public function husnanWUpdate($tableName, Array $params, $where)
    {
        ...
        $sql = "UPDATE ".$tableName." SET ".$signs." WHERE ".$where. "  AND flag_operator_opd IS NOT NULL AND flag_kepala_opd IS NOT NULL AND flag_operator_kota IS NOT NULL";
