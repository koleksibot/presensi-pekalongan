...
//        $result['query'] = '';
    ...
    public function getModerasi($data, $datalap = []) {
        ...
        elseif ($data['tingkat'] > 2 && isset($datalap['sah_kepala_opd'])) {
            if ($data['tingkat'] == 4)
                $plus = " OR flag_operator_kota IS NULL";
            elseif ($data['tingkat'] == 5)
                $plus = " OR flag_kepala_kota IS NULL";
            $status = "AND (flag_kepala_opd IS NULL OR flag_kepala_opd = '1'".$plus.")";
        }*/
    ...
    public function getArraymodAll($input, $laporan = []) {
        ...
        $dataArr = []; $catatan = 0;
        foreach ($data['value'] as $mod) {
            $cat_kepalakota = trim($mod['catatan_kepala_kota']);
            if (!empty($mod['catatan_kepala_kota']) && $mod['catatan_kepala_kota'] != '')
                $catatan++;
            ...
            if ($input['tingkat'] == 5 && $mod['flag_kepala_kota'] != 2) 
                $verified = null;
    ...
    public function getDataPersonilTpp($data) {
        ...
        $query = 'SELECT pin_absen, v.*, pp.npwp
