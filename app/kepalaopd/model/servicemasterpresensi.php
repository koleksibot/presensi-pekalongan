...
class servicemasterpresensi extends system\Model {
    ...
    public function getTabelPanduan($data) {
        ...
        $query = 'SELECT * FROM tb_panduan' . $q_cari . ' AND access_level IN ("kepalaopd", "public")';
        $idKey = array();
    ...
    {
        $dataArr = $this->getData('SELECT * FROM tb_teks WHERE lokasi="BERANDA" AND kepala_opd = 1 AND tampil = 1', array());
        $tempel = []; $popup = [];
