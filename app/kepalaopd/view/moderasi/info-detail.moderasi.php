            ...
            <div class="col s12 m11">
                <div class="card">
                    <div class="card-image">
                        <img src="http://simpeg.pekalongankota.go.id/upload/foto/<?= $info["nipbaru"] ?>-FOTO.jpg">
                    ...
                    <div id="divCardAction" class="card-action center">
                    <?php if (((!is_null($info["flag_operator_kota"]) && !is_null($info["flag_kepala_kota"]))) && !is_null($info["flag_operator_opd"])): ?>
                        <textarea id="txtCatatan" class="materialize-textarea" placeholder="Berikanlah catatan final bila diperlukan"><?= $info["catatan_final_kepala_opd"] ?></textarea>
                        <label for="txtCatatan" class="active">Tentukan Status Final Moderasinya</label>
                        <br><br>
                        <a class="btn-batalkan-mod btn-floating waves-effect waves-light black" title="saya batalkan" mid="<?= $info["id"] ?>"><i class="material-icons">delete</i></a>
                        <a class="btn-sahkan-mod btn-floating waves-effect waves-light orange" title="saya sahkan" mid="<?= $info["id"] ?>"><i class="material-icons">thumb_up</i> Sahkan!</a>
                    <?php elseif(is_null($info["flag_operator_kota"]) && is_null($info["flag_kepala_kota"]) && !is_null($info["flag_operator_opd"])): ?>
                        <textarea id="txtCatatan" class="materialize-textarea" placeholder="Berikanlah catatan bila diperlukan"><?= $info["catatan_kepala_opd"] ?></textarea>
                        <label for="txtCatatan" class="active">Tentukan Status Verifikasinya</label>
                        <br><br>
                        <a class="btn-tolak-mod btn-floating waves-effect waves-light red" title="saya tolak" mid="<?= $info["id"] ?>"><i class="material-icons">close</i></a>
                        <a class="btn-terima-mod btn-floating waves-effect waves-light green" title="saya terima" mid="<?= $info["id"] ?>"><i class="material-icons">check</i></a>
                    <?php else: ?>
