function husnanw_moderasi_main_admin_sistem(route, dateLimit) {
    disableInputs(true);
    var dateAwal = moment().startOf('month').format('DD-MM-YYYY');
...
console.log($("option:selected",this).prop("kategori"));
    ...
    $("#selOpd").change(function () {
        ajaxRequester(route + "/getDaftarPegawaiModerasi/" + $(this).val(), {}, function (result) {
            ...
            getSelectedPns(route, $('#selDaftarPns').val());
            $(".tglmoderasi").removeClass("disinput");
        }, "get", "json");
    });

    $("#selDaftarPns").change(function () {
       getSelectedPns(route, $(this).val()); 
    });

    /*
    ajaxRequester(route + "/getDaftarPns", {kodeLokasi: $(this).val()}, function (result) {
            $("#hidPin").val(result.pin_absen);
            $("#spNamaPemohon").text(result.nama_lengkap);
            $("#spNipPemohon").text(result.nipbaru);
            $("#divFotoPemohon").html('<img src="http://simpeg.pekalongankota.go.id/upload/foto/'+result.nipbaru+'-FOTO.jpg" class="responsive-img" alt="foto">');
        }, "get", "json");
    */
...
function getSelectedPns(route, nip) {
    ajaxRequester(route + "/getSelectedPns/" + nip, {}, function (result) {
        $("#hidPin").val(result.pin_absen);
        $("#spNamaPemohon").text(result.nama_lengkap);
        $("#spNipPemohon").text(result.nipbaru);
        $("#divFotoPemohon").html('<img src="http://simpeg.pekalongankota.go.id/upload/foto/'+result.nipbaru+'-FOTO.jpg" class="responsive-img" alt="foto">');
...
function getLimitDateAwal(limitDate)
{
    ...
    if (currMonth === 0 && currDate <= limitDate) {
